...
export default async function HomePage() {
  ...
  const month = now.getMonth(); // 0 = ม.ค. ... 4 = พ.ค.
  ...
  const [trendingAnime, airingSchedule, seasonalAnime] = await Promise.all([
    getTrendingAnime(6),
    getAiringSchedule(weekStart, weekEnd, 1, 60),
    getSeasonalAnimeList(currentSeason, currentYear, 12),
  ]);
  ...
  const currentSeasonCustom = customAnimeList.filter(
    (anime) => anime.season === currentSeason && anime.seasonYear === currentYear
  );

  // ผสานข้อมูล Custom เข้ากับรายการจาก API
  const mergedSeasonalAnime = [...currentSeasonCustom, ...seasonalAnime];

  // ฟอร์แมตชื่อซีซันเป็นตัวพิมพ์แรกใหญ่
  const formattedSeason = currentSeason.charAt(0) + currentSeason.slice(1).toLowerCase();
